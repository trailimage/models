...
class EXIF {
    ...
    sanitize() {
        ...
        if (tools_1.is.value(this.artist) &&
            tools_1.is.value(config_1.config.artistsToNormalize) &&
            config_1.config.artistsToNormalize.test(this.artist)) {
 ...
 */
const camera = (text) => tools_1.is.empty(text)
    ? ''
    : text
        .replace('NIKON', 'Nikon')
 ...
 * Normalize lens name.
 */
const lens = (text, camera) => tools_1.is.empty(text)
    ? ''
    : text
        ...
        .replace('35.0 mm f/2.0', /D700/.test(camera)
 ...
 */
const software = (text) => tools_1.is.empty(text)
    ? ''
    : text
 ...
 */
const compensation = (text) => text == '0' ? 'No' : text.toString();
